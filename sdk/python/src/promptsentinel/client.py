...
class PromptSentinel(BaseClient):
    ...
    def add_threat_feed(self, feed_data: dict[str, Any]) -> dict[str, Any]:
        """Add a new threat intelligence feed."""
        url = urljoin(self.base_url, "/api/v1/threat/feeds")
        response = self.client.post(url, json=feed_data)
        return self._handle_response(response)

    def list_threat_feeds(self) -> list[dict[str, Any]]:
        """List all threat intelligence feeds."""
        url = urljoin(self.base_url, "/api/v1/threat/feeds")
        response = self.client.get(url)
        return self._handle_response(response)

    def get_threat_feed(self, feed_id: str) -> dict[str, Any]:
        """Get a specific threat feed by ID."""
        url = urljoin(self.base_url, f"/api/v1/threat/feeds/{feed_id}")
        response = self.client.get(url)
        return self._handle_response(response)

    def update_threat_feed(self, feed_id: str) -> dict[str, Any]:
        """Manually trigger update of a threat feed."""
        url = urljoin(self.base_url, f"/api/v1/threat/feeds/{feed_id}/update")
        response = self.client.post(url)
        return self._handle_response(response)

    def remove_threat_feed(self, feed_id: str) -> bool:
    ...
    def get_threat_indicators(
        self, technique: str | None = None, min_confidence: float = 0.0, limit: int = 100
    ) -> list[dict[str, Any]]:
        """Get active threat indicators."""
        url = urljoin(self.base_url, "/api/v1/threat/indicators")
    ...
    def search_threat_indicators(self, query: str, limit: int = 100) -> list[dict[str, Any]]:
        """Search threat indicators."""
        url = urljoin(self.base_url, "/api/v1/threat/indicators/search")
        params = {"query": query, "limit": limit}
        response = self.client.get(url, params=params)
        return self._handle_response(response)

    def report_false_positive(self, indicator_id: str, reason: str | None = None) -> dict[str, Any]:
        """Report a false positive for a threat indicator."""
        url = urljoin(self.base_url, f"/api/v1/threat/indicators/{indicator_id}/false-positive")
        data = {"reason": reason} if reason else {}
        response = self.client.post(url, json=data)
        return self._handle_response(response)

    def confirm_true_positive(
        self, indicator_id: str, details: str | None = None
    ) -> dict[str, Any]:
        """Confirm a true positive detection."""
        url = urljoin(self.base_url, f"/api/v1/threat/indicators/{indicator_id}/true-positive")
    ...
    def get_threat_statistics(self) -> dict[str, Any]:
        ...
        url = urljoin(self.base_url, "/api/v1/threat/statistics")
        response = self.client.get(url)
        return self._handle_response(response)
...
class AsyncPromptSentinel(BaseClient):
    ...
    # Threat Intelligence Methods (Async)
    async def add_threat_feed(self, feed_data: dict[str, Any]) -> dict[str, Any]:
        """Add a new threat intelligence feed (async)."""
        url = urljoin(self.base_url, "/api/v1/threat/feeds")
        response = await self.client.post(url, json=feed_data)
        return self._handle_response(response)

    async def list_threat_feeds(self) -> list[dict[str, Any]]:
        """List all threat intelligence feeds (async)."""
        url = urljoin(self.base_url, "/api/v1/threat/feeds")
        response = await self.client.get(url)
        return self._handle_response(response)

    async def get_threat_feed(self, feed_id: str) -> dict[str, Any]:
        """Get a specific threat feed by ID (async)."""
        url = urljoin(self.base_url, f"/api/v1/threat/feeds/{feed_id}")
        response = await self.client.get(url)
        return self._handle_response(response)

    async def update_threat_feed(self, feed_id: str) -> dict[str, Any]:
        """Manually trigger update of a threat feed (async)."""
        url = urljoin(self.base_url, f"/api/v1/threat/feeds/{feed_id}/update")
        response = await self.client.post(url)
        return self._handle_response(response)

    async def remove_threat_feed(self, feed_id: str) -> bool:
        """Remove a threat feed (async)."""
        url = urljoin(self.base_url, f"/api/v1/threat/feeds/{feed_id}")
    ...
    async def get_threat_indicators(
        ...
        """Get active threat indicators (async)."""
        url = urljoin(self.base_url, "/api/v1/threat/indicators")
        params = {"limit": limit, "min_confidence": min_confidence}
        if technique:
            params["technique"] = technique
    ...
    async def search_threat_indicators(self, query: str, limit: int = 100) -> list[dict[str, Any]]:
        """Search threat indicators (async)."""
        url = urljoin(self.base_url, "/api/v1/threat/indicators/search")
        params = {"query": query, "limit": limit}
        response = await self.client.get(url, params=params)
        return self._handle_response(response)
    ...
    async def report_false_positive(
        self, indicator_id: str, reason: str | None = None
    ) -> dict[str, Any]:
        """Report a false positive for a threat indicator (async)."""
        url = urljoin(self.base_url, f"/api/v1/threat/indicators/{indicator_id}/false-positive")
        data = {"reason": reason} if reason else {}
        response = await self.client.post(url, json=data)
        return self._handle_response(response)
    ...
    async def confirm_true_positive(
        ...
        """Confirm a true positive detection (async)."""
        url = urljoin(self.base_url, f"/api/v1/threat/indicators/{indicator_id}/true-positive")
        data = {"details": details} if details else {}
        response = await self.client.post(url, json=data)
        return self._handle_response(response)
    ...
    async def get_threat_statistics(self) -> dict[str, Any]:
        """Get overall threat intelligence statistics (async)."""
        url = urljoin(self.base_url, "/api/v1/threat/statistics")
        response = await self.client.get(url)
        return self._handle_response(response)
