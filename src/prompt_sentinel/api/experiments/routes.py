...
from .schemas import (
    CreateExperimentRequest, UpdateExperimentRequest, ExperimentStatusRequest,
    ExperimentSummary, ExperimentDetails, ExperimentMetricsQuery, 
    ExperimentMetricsResponse, ExperimentAssignmentRequest, 
    ExperimentAssignmentResponse, RecordMetricRequest, BatchMetricsRequest,
    ExperimentResultsQuery, ExperimentResultsResponse, ExperimentListQuery,
...
async def list_experiments(
    ...
    experiment_type: Optional[ExperimentType] = Query(None, alias="type", description="Filter by type"),
    ...
    try:
        ...
        for exp_data in paginated:
            ...
            summary = ExperimentSummary(
                ...
                created_at=datetime.fromisoformat(exp_data.get("created_at", datetime.utcnow().isoformat())),
                ...
                target_percentage=exp_status.get("runtime_metrics", {}).get("target_percentage", 0.0) if exp_status else 0.0,
                total_assignments=exp_status.get("assignment_stats", {}).get("total_assignments", 0) if exp_status else 0,
                health_score=exp_status.get("safety_report", {}).get("health_score", 1.0) if exp_status else 1.0
            )
...
async def change_experiment_status(
    ...
    try:
        if request.status == ExperimentStatus.RUNNING:
            ...
            raise HTTPException(status_code=400, detail=f"Status transition to {request.status} not supported")
        
        if not success:
            raise HTTPException(status_code=404, detail="Experiment not found or cannot change status")
        
        logger.info("Experiment status changed via API",
        ...
        logger.error("Failed to change experiment status",
...
async def assign_user_to_experiment(
    ...
    try:
        ...
        # Get assignment
        assignment = await manager.assign_user(
            request.user_id, experiment_id, context
        )
        
        if assignment:
            # Get variant configuration
            variant_config = await manager.get_variant_config(
                request.user_id, experiment_id
            )
...
async def record_experiment_metric(
    ...
    try:
        ...
        logger.error("Failed to record experiment metric",
                    experiment_id=experiment_id, error=str(e))
        raise HTTPException(status_code=500, detail="Failed to record metric")


@router.post("/{experiment_id}/metrics/batch", response_model=dict, 
...
async def delete_experiment(
    ...
    try:
        ...
        if status.get("is_active", False):
            raise HTTPException(
                status_code=400, 
                detail="Cannot delete active experiment. Stop it first."
            )
...
@router.get("/stats/overview", response_model=ExperimentStatsResponse, 
