...
# Endpoints
@router.post(
    "/feedback", response_model=FeedbackResponse, summary="Submit feedback for detection event"
)
async def submit_feedback(request: FeedbackRequest):
    ...
    try:
        ...
        if success:
            logger.info("Feedback submitted", event_id=request.event_id, label=request.user_label)

            return FeedbackResponse(
                success=True, message="Feedback recorded successfully", event_id=request.event_id
            )
        else:
            return FeedbackResponse(
                success=False, message="Event not found in buffer", event_id=request.event_id
            )
...
async def discover_patterns(request: PatternDiscoveryRequest, manager=Depends(get_pattern_manager)):
    ...
    try:
        ...
        logger.info("Pattern discovery completed", patterns_found=len(patterns))
...
async def get_pattern(
    ...
    return PatternResponse(
        ...
        performance={
            ...
            "total_matches": managed.performance.total_matches,
            "last_match": (
                managed.performance.last_match.isoformat()
                if managed.performance.last_match
                else None
            ),
        },
    )
...
async def promote_pattern(
    ...
    if success:
        ...
        raise HTTPException(status_code=400, detail="Pattern cannot be promoted or not found")
...
@router.get("/metrics", response_model=dict, summary="Get ML system metrics")
async def get_ml_metrics(manager=Depends(get_pattern_manager)):
...
@router.delete("/patterns/{pattern_id}", response_model=dict, summary="Delete a pattern")
async def delete_pattern(
    ...
    return {"success": True, "pattern_id": pattern_id, "message": "Pattern deleted successfully"}
