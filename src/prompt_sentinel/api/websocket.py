...
class StreamingDetector:
    ...
    async def process_detection(self, request_data: dict, client_id: str) -> dict:
        ...
        try:
            ...
            if self.router and request_data.get("use_intelligent_routing", False):
                ...
                # Direct detection
                response = await self.detector.detect(
                    messages=messages,
                    check_format=check_format
                )
            ...
            logger.error("Detection processing error", 
                        client_id=client_id, error=str(e))
            return {
                "type": "error",
    ...
    async def process_analysis(self, request_data: dict, client_id: str) -> dict:
        ...
        try:
            ...
            detection_response = await self.detector.detect(
                messages=messages,
                check_format=True
            )
...
async def handle_websocket_connection(
    ...
    if client:
        connection_manager.connection_metadata[client_id].update({
            "authenticated": client.is_authenticated if hasattr(client, 'is_authenticated') else False,
