    ...
    def _init_patterns(self):
        ...
        self.patterns = {
            PIIType.CREDIT_CARD: [
                ...
                (r'\b(?:4[0-9]{12}(?:[0-9]{3})?|5[1-5][0-9]{14}|3[47][0-9]{13}|6(?:011|5[0-9][0-9])[0-9]{12})\b', 0.8),
            ...
            PIIType.IP_ADDRESS: [
                # IPv4
                (r'\b(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\b', 0.9),
            ...
            PIIType.DATE_OF_BIRTH: [
                ...
                (r'\b(?:DOB|dob|Date of Birth|date of birth)[\s:]+\d{1,2}[-/]\d{1,2}[-/]\d{2,4}\b', 0.9),
            ...
            for pattern, confidence in self.patterns[pii_type]:
                for match in re.finditer(pattern, text, re.IGNORECASE):
                    ...
                    if pii_type == PIIType.CREDIT_CARD:
                        if not self._validate_credit_card(matched_text.replace('-', '').replace(' ', '')):
                            continue
