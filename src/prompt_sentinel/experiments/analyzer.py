...
class ExperimentResult:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        improvement = (
            ...
            / self.control_metrics.mean * 100
            if self.control_metrics.mean != 0 else 0
        )
...
class StatisticalAnalyzer:
    ...
    async def analyze_experiment(
        ...
        for metric_name, variant_data in metric_data.items():
            if len(variant_data) < 2:
                logger.warning("Insufficient variants for analysis",
            ...
            if not control_id or not treatment_ids:
                logger.warning("Could not identify control/treatment variants",
                             experiment_id=experiment_id,
                             metric=metric_name)
                continue
            ...
            for treatment_id in treatment_ids:
                ...
                if (len(control_values) < min_sample_size or 
    ...
    async def _analyze_metric(
        ...
        try:
            ...
            logger.error("Failed to analyze metric",
                        experiment_id=experiment_id,
                        metric=metric_name,
                        error=str(e))
            return None
    ...
    def _select_test_type(
        ...
        control_data: MetricData,
        treatment_data: MetricData
    ) -> SignificanceTest:
