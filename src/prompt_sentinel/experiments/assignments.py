...
class AssignmentService:
    ...
    async def assign_user(
        ...
        if not experiment.is_active():
            logger.debug("Experiment not active", 
        ...
        if not self._passes_targeting_filters(context, experiment):
            logger.debug("User excluded by targeting filters",
                        user_id=context.user_id,
                        experiment_id=experiment.id)
            return None
        ...
        if not self._is_in_experiment_sample(context.user_id, experiment):
            logger.debug("User not in experiment sample",
                        user_id=context.user_id,
                        experiment_id=experiment.id)
            return None
    ...
    async def bulk_assign_users(
        ...
        """
        tasks = [
            self.assign_user(context, experiment, strategy)
        ...
        for i, assignment in enumerate(assignments):
            if isinstance(assignment, Exception):
                logger.error("Failed to assign user",
    ...
    async def _get_existing_assignment(
        ...
        if cache_manager and cache_manager.connected:
            try:
                ...
                logger.warning("Failed to get assignment from cache",
    ...
    async def _cache_assignment(self, assignment: ExperimentAssignment):
        ...
        if cache_manager and cache_manager.connected:
            try:
                ...
                logger.warning("Failed to cache assignment",
                             error=str(e), cache_key=cache_key)
    ...
    async def get_assignment_stats(
        ...
        for key, assignment in self.assignments_cache.items():
            if assignment.experiment_id == experiment_id:
                ...
                variant_id = assignment.variant_id
                stats["variant_counts"][variant_id] = \
                    stats["variant_counts"].get(variant_id, 0) + 1
    ...
    async def invalidate_assignments(self, experiment_id: str):
        ...
        if cache_manager and cache_manager.connected:
            try:
                ...
                logger.info("Invalidated Redis assignments", 
                          experiment_id=experiment_id)
            except Exception as e:
                logger.warning("Failed to invalidate Redis assignments",
                             experiment_id=experiment_id, error=str(e))
        
        # Clear from memory cache
        keys_to_remove = [
            key for key in self.assignments_cache.keys()
            if key.startswith(f"assignment:{experiment_id}:")
        ]
        for key in keys_to_remove:
            del self.assignments_cache[key]
        
        logger.info("Invalidated experiment assignments",
                   experiment_id=experiment_id,
