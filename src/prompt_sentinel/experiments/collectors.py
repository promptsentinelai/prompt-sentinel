...
class MetricsCollector:
    ...
    async def record_experiment_metric(
        ...
        logger.debug("Experiment metric recorded",
                    experiment_id=experiment_id,
                    variant_id=variant_id,
                    metric=metric_name,
                    value=value)
    ...
    async def get_aggregated_metrics(
        ...
        # Check cache
        cache_key = f"{variant_id}:{metric_name}"
        if (experiment_id in self.aggregation_cache and 
            cache_key in self.aggregation_cache[experiment_id]):
    ...
    async def get_variant_performance(
        ...
        if variant_metrics:
            ...
            performance["summary"]["avg_events_per_user"] = (
                len(variant_metrics) / len(set(m.user_id for m in variant_metrics))
            )
            performance["summary"]["first_event"] = min(m.timestamp for m in variant_metrics).isoformat()
            performance["summary"]["last_event"] = max(m.timestamp for m in variant_metrics).isoformat()
    ...
    async def record_detection_metrics(
        ...
        # Binary metrics
        await self.record_experiment_metric(
            experiment_id, variant_id, "blocked", 
            1.0 if verdict == "block" else 0.0, user_id
        )
        
        await self.record_experiment_metric(
            experiment_id, variant_id, "cache_hit", 
            1.0 if cache_hit else 0.0, user_id
        )
        
        await self.record_experiment_metric(
            experiment_id, variant_id, "pii_detected", 
            1.0 if pii_detected else 0.0, user_id
        )
        ...
        logger.debug("Detection metrics recorded",
                    experiment_id=experiment_id,
                    variant_id=variant_id,
    ...
    async def _cache_metric(self, metric: ExperimentMetric):
        ...
        try:
            cache_key = f"experiment_metric:{metric.experiment_id}:{metric.variant_id}:{metric.metric_name}"
            metric_data = {
                "value": metric.value,
                "timestamp": metric.timestamp.isoformat(),
                "user_id": metric.user_id,
