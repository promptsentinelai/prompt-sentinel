...
class ExperimentDatabase:
    ...
    async def initialize(self):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.execute("CREATE INDEX IF NOT EXISTS idx_experiments_status ON experiments(status)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_experiments_type ON experiments(type)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_metrics_experiment ON experiment_metrics(experiment_id)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_metrics_variant ON experiment_metrics(variant_id)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_metrics_timestamp ON experiment_metrics(timestamp)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_assignments_experiment ON experiment_assignments(experiment_id)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_assignments_user ON experiment_assignments(user_id)")
    ...
    async def save_experiment(self, config: ExperimentConfig) -> bool:
        ...
        try:
            ...
            logger.error("Failed to save experiment", 
                        experiment_id=config.id, error=str(e))
            return False
    ...
    async def delete_experiment(self, experiment_id: str) -> bool:
        ...
        try:
            async with aiosqlite.connect(self.db_path) as db:
                ...
                await db.execute("DELETE FROM safety_violations WHERE experiment_id = ?", (experiment_id,))
                await db.execute("DELETE FROM assignment_events WHERE experiment_id = ?", (experiment_id,))
    ...
    async def save_safety_violation(self, violation: GuardrailViolation) -> bool:
        ...
        try:
            ...
            logger.debug("Safety violation saved", 
                        experiment_id=violation.experiment_id)
            return True
            
        except Exception as e:
            logger.error("Failed to save safety violation", 
                        experiment_id=violation.experiment_id, error=str(e))
            return False
