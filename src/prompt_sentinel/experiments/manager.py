...
from .config import (
    ExperimentConfig, ExperimentVariant, ExperimentStatus, 
    ExperimentAssignment, ExperimentMetadata, ExperimentType
)
...
class ExperimentManager:
    ...
    async def start_experiment(self, experiment_id: str) -> bool:
        ...
        if config.status not in [ExperimentStatus.DRAFT, ExperimentStatus.SCHEDULED]:
            raise ExperimentError(
                f"Cannot start experiment in status: {config.status}"
            )
        ...
        execution = ExperimentExecution(
            ...
            variant_configs={
                variant.id: variant.config for variant in config.variants
            },
            assignment_cache={}
        )
    ...
    async def record_detection_result(
        ...
        await self.record_metric(
            experiment_id, user_id, "response_time_ms", 
            detection_response.processing_time_ms
        )
        ...
        await self.record_metric(
            experiment_id, user_id, "blocked", 
            1.0 if detection_response.verdict == "block" else 0.0
        )
        
        await self.record_metric(
            experiment_id, user_id, "pii_detected", 
            1.0 if detection_response.pii_detected else 0.0
        )
        ...
        if routing_metadata:
            if "strategy" in routing_metadata:
                await self.record_metric(
                    experiment_id, user_id, "strategy_latency",
    ...
    async def get_experiment_status(
        ...
        assignment_stats = await self.assignment_service.get_assignment_stats(
            experiment_id
        )
        ...
        if experiment_id in self.active_experiments:
            ...
            runtime_metrics = {
                ...
                "last_analysis": execution.last_analysis.isoformat() if execution.last_analysis else None
            }
    ...
    async def _check_auto_promotion(
        ...
        if significant_improvements:
            ...
            logger.info("Experiment has significant improvements",
                       experiment_id=experiment_id,
    ...
    async def _handle_safety_violation(self, violation: GuardrailViolation):
        ...
        logger.warning("Safety violation in experiment",
