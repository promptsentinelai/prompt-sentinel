...
class SafetyControls:
    ...
    async def register_experiment(
        ...
        logger.info("Experiment registered for safety monitoring",
                   experiment_id=experiment.id,
    ...
    async def unregister_experiment(self, experiment_id: str):
        ...
        logger.info("Experiment unregistered from safety monitoring",
    ...
    async def check_experiment_safety(
        ...
        for guardrail in experiment.guardrails:
            ...
            violation = self._check_guardrail(
                experiment_id, guardrail, actual_value
            )
    ...
    async def emergency_stop(self, experiment_id: str, reason: str):
        ...
        """
        if experiment_id not in self.active_experiments:
            logger.warning("Cannot emergency stop unknown experiment",
                         experiment_id=experiment_id)
            return
        ...
        logger.critical("Emergency experiment stop",
                       experiment_id=experiment_id,
                       reason=reason)
    ...
    def get_safety_report(
        ...
        metrics = self.safety_metrics[experiment_id]
        recent_violations = [
            v for v in self.guardrail_violations
    ...
    async def _monitor_experiment(
        ...
        try:
            while experiment_id in self.active_experiments:
                ...
                # Check guardrails
                violations = await self.check_experiment_safety(
                    experiment_id, metric_data
                )
            ...
            logger.info("Experiment monitoring cancelled",
                       experiment_id=experiment_id)
        except Exception as e:
            logger.error("Experiment monitoring failed",
                        experiment_id=experiment_id,
                        error=str(e))
    ...
    async def _handle_violation(self, violation: GuardrailViolation):
        ...
        # Log violation
        logger.warning("Guardrail violation",
                      experiment_id=experiment_id,
    ...
    async def _pause_experiment(self, experiment_id: str, reason: str):
        ...
        logger.warning("Experiment paused due to safety violation",
                      experiment_id=experiment_id,
                      reason=reason)
    ...
    async def _terminate_experiment(self, experiment_id: str, reason: str):
        ...
        logger.error("Experiment terminated due to safety violation",
                    experiment_id=experiment_id,
                    reason=reason)
    ...
    async def _reduce_experiment_traffic(self, experiment_id: str):
        ...
        logger.warning("Reduced experiment traffic",
                      experiment_id=experiment_id,
                      new_percentage=experiment.target_percentage)
    
    async def _update_health_score(
        self, experiment_id: str, violations: List[GuardrailViolation]
    ):
