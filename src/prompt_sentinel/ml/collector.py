...
class PatternCollector:
    ...
    async def shutdown(self):
        ...
        logger.info("Pattern collector shutdown",
    ...
    async def collect_event(
        ...
        logger.debug("Detection event collected",
    ...
    async def add_feedback(
        ...
        for event in self.event_buffer:
            if event.event_id == event_id:
                ...
                logger.info("Feedback added to event",
                           event_id=event_id,
                           label=user_label)
                return True
    ...
    def get_events_for_clustering(
        ...
        for event in self.event_buffer:
            ...
            # Check criteria
            if (event.confidence >= min_confidence and 
                event.verdict in verdicts_to_check):
                events.append(event)
        ...
        while self._running:
            try:
                ...
                self.event_buffer = deque(
                    (e for e in self.event_buffer if e.timestamp > cutoff),
