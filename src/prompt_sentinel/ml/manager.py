...
class PatternManager:
    ...
    async def discover_patterns(self) -> List[ExtractedPattern]:
        ...
        if len(events) < self.clustering_engine.min_cluster_size:
            logger.info("Not enough events for clustering", 
        ...
        # Run clustering
        clusters = await self.clustering_engine.cluster_events(
            feature_matrix, events
        )
        ...
        merged_patterns = self.pattern_extractor.merge_similar_patterns(
            all_patterns
        )
        ...
        for pattern in merged_patterns:
            if pattern.pattern_id not in self.patterns:
                ...
                self.patterns_by_status[PatternStatus.CANDIDATE].append(
                    pattern.pattern_id
                )
    ...
    async def promote_pattern(self, pattern_id: str) -> bool:
        ...
        if managed.status in [PatternStatus.RETIRED, PatternStatus.REJECTED]:
            logger.warning("Cannot promote pattern", 
    ...
    def get_pattern_statistics(self) -> Dict[str, Any]:
        ...
        """
        status_counts = {
            status.value: len(self.patterns_by_status[status])
        ...
        if active_patterns:
            avg_precision = sum(p.performance.precision for p in active_patterns) / len(active_patterns)
    ...
    async def _check_pattern_status(self, managed: ManagedPattern):
        ...
        total_evaluations = (perf.true_positives + perf.false_positives +
                           perf.true_negatives + perf.false_negatives)
        
        # Check for promotion
        if (managed.status == PatternStatus.CANDIDATE and
            total_evaluations >= self.min_samples_for_promotion):
            
            if (perf.precision >= self.min_precision and
                perf.recall >= self.min_recall):
                # Promote to testing
                managed.status = PatternStatus.TESTING
                self.patterns_by_status[PatternStatus.CANDIDATE].remove(
                    managed.pattern.pattern_id
                )
                self.patterns_by_status[PatternStatus.TESTING].append(
                    managed.pattern.pattern_id
                )
                logger.info("Pattern promoted to testing",
        ...
        # Check active patterns for retirement
            ...
            if (total_evaluations > 1000 and
                (perf.precision < self.min_precision * 0.8 or
                 perf.false_positives > perf.true_positives * 2)):
    ...
    async def _save_patterns(self):
        ...
        try:
            patterns_data = [
                managed.to_dict() 
                for managed in self.patterns.values()
            ]
