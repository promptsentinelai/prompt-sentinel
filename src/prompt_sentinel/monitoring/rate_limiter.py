...
class RateLimiter:
    ...
    async def check_rate_limit(self,
                              ...
                              tokens: int = 0,
                              priority: Priority = Priority.NORMAL) -> Tuple[bool, Optional[float]]:
        ...
        if client_id:
            ...
            if not client_request_bucket.can_consume(1):
                ...
                self.rejected_requests += 1
                logger.debug(
                    "Client request rate limit exceeded",
                    client=client_id,
                    wait_time=wait_time
                )
                return False, wait_time
    ...
    async def consume_tokens(self,
        ...
        # Update load metrics
        self.current_load = 1.0 - (self.global_request_bucket.tokens / self.global_request_bucket.capacity)
