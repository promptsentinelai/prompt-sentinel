...
class UsageTracker:
    ...
    def __init__(self, 
                 ...
                 retention_hours: int = 24 * 7):  # 1 week default
    ...
    async def _update_metrics(self, api_call: ApiCall):
        ...
        self.metrics.avg_latency_ms = (
            (self.metrics.avg_latency_ms * (total - 1) + api_call.latency_ms) / total
        )
        ...
        if api_call.provider != Provider.CACHE:
            ...
            if api_call.success:
                pm["success_rate"] = (
                    (pm["success_rate"] * (pm["requests"] - 1) + 1) / pm["requests"]
                )
            else:
                pm["success_rate"] = (
                    (pm["success_rate"] * (pm["requests"] - 1)) / pm["requests"]
                )
        
        # Calculate rate metrics
        time_since_start = (now - self.api_calls[0].timestamp).total_seconds() if self.api_calls else 1
    ...
    def get_metrics(self, 
                    time_window: Optional[timedelta] = None) -> UsageMetrics:
    ...
    def get_cost_breakdown(self, 
                          group_by: str = "provider") -> Dict[str, float]:
    ...
    def get_usage_trend(self, 
        ...
        for call in self.api_calls:
            ...
            p["avg_latency"] = (
                (p["avg_latency"] * (p["requests"] - 1) + call.latency_ms) 
                / p["requests"]
            )
        ...
        trend = [
            {"period": k, **v} 
            for k, v in sorted(periods.items(), reverse=True)[:limit]
        ]
    ...
    def clear_old_data(self, retention_hours: Optional[int] = None):
        ...
        logger.info(
            "Cleared old usage data",
            retention_hours=retention,
            remaining_calls=len(self.api_calls)
        )