...
class ComplexityAnalyzer:
    ...
    def analyze(self, messages: List[Message]) -> ComplexityScore:
        ...
        if any(risk in risk_indicators for risk in [
    ...
    def _calculate_pattern_complexity(self, content: str) -> Tuple[float, List[RiskIndicator]]:
        ...
        override_count = sum(
            1 for pattern in override_patterns 
    ...
    def _calculate_semantic_complexity(self, messages: List[Message]) -> Tuple[float, List[RiskIndicator]]:
        ...
        role_manipulation_indicators = [
            "you are", "you're now", "act as", "pretend to be",
            "roleplay", "simulate", "impersonate", "behave as"
        ]
        ...
        for msg in messages:
            if msg.role == Role.USER:
                ...
                manipulation_count = sum(
                    1 for indicator in role_manipulation_indicators
        ...
        system_in_user = any(
            ...
            any(keyword in msg.content.lower() for keyword in ["system:", "[system]", "<system>"])
            for msg in messages
        )
        if system_in_user:
            score += 0.3
    ...
    def _detect_obfuscation(self, content: str) -> bool:
        ...
        suspicious_substitutions = sum(
            1 for char, substitute in homoglyphs.items()
