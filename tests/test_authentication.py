...
class TestAuthModes:
    ...
    def test_auth_mode_required(self, client):
        ...
        with patch('prompt_sentinel.auth.dependencies.get_auth_config') as mock_config:
            ...
            assert "required" in response.json()["detail"].lower() or "denied" in response.json()["detail"].lower()
...
class TestBypassRules:
    ...
    def test_network_bypass(self):
        ...
        config = AuthConfig(
            mode=AuthMode.REQUIRED,
            bypass_networks=["10.0.0.0/8", "192.168.0.0/16"]
        )
    ...
    def test_header_bypass(self):
        ...
        config = AuthConfig(
            mode=AuthMode.REQUIRED,
            bypass_headers={"X-Internal": "true", "X-Service": "gateway"}
        )
