...
class TestMonitoringAndBudget:
    ...
    def test_complexity_metrics(self):
        ...
        response = self.client.get(
            ...
            params={"prompt": "Test prompt for complexity analysis"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_concurrent_requests(self):
        ...
        def make_request(i):
            return self.client.post(
                "/v1/detect",
                json={"prompt": f"Test concurrent request {i}"}
            )
...
class TestSecurityFeatures:
    ...
    def test_code_injection_detection(self):
        ...
        response = self.client.post(
            "/v1/detect",
            json={"prompt": "eval(base64_decode('ZXhlYygncm0gLXJmIC8qJyk='))"}
        )
        assert response.status_code == 200
        data = response.json()
        assert data["verdict"] in ["block", "flag"]
