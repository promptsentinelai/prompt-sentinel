...
class TestPerformanceBenchmarks:
    ...
    def test_cache_hit_performance(self):
        ...
        payload = {"messages": [{"role": "user", "content": prompt}], "use_cache": True}
    ...
    def test_batch_processing_performance(self):
        ...
        for size in batch_sizes:
            prompts = [{"id": str(i), "prompt": f"Test prompt {i}"} for i in range(size)]
            ...
            if response.status_code == 200:
                ...
                results[size] = {"total_ms": elapsed, "per_prompt_ms": per_prompt}

        print("\nBatch Processing Performance:")
        for size, metrics in results.items():
            print(
                f"  Batch size {size}: {metrics['total_ms']:.2f}ms total, {metrics['per_prompt_ms']:.2f}ms per prompt"
            )
...
class TestLoadTesting:
    ...
    def make_concurrent_requests(self, num_requests: int, num_workers: int = 10) -> dict[str, Any]:
        ...
        def make_request(i):
            try:
                start = time.perf_counter()
                response = self.client.post("/v1/detect", json={"prompt": f"Load test prompt {i}"})
                elapsed = (time.perf_counter() - start) * 1000
            ...
            with ThreadPoolExecutor(max_workers=requests_per_second) as executor:
                ...
                for i in range(requests_per_second):
                    future = executor.submit(
                        self.client.post, "/v1/detect", json={"prompt": f"Sustained load test {i}"}
                    )
...
class TestMemoryAndResourceUsage:
    ...
    def test_memory_leak_detection(self):
        ...
        for i in range(100):
            response = self.client.post(
                "/v1/detect", json={"prompt": f"Memory test {i}" * 100}  # Larger prompts
            )
    ...
    def test_large_payload_handling(self):
        ...
        for size in sizes:
            large_prompt = "a" * size
            response = self.client.post("/v1/detect", json={"prompt": large_prompt})
