...
class TestHeuristicPerformance:
    ...
    def test_bulk_message_performance(self):
        ...
        messages = [
            Message(role=Role.USER, content=f"Message {i}: {'test' * (i % 10)}") for i in range(100)
        ]
    ...
    def test_concurrent_detection_performance(self):
        ...
        async def benchmark_concurrent():
            messages = [Message(role=Role.USER, content=f"Concurrent test {i}") for i in range(100)]
...
class TestLLMClassifierPerformance:
    ...
    async def test_provider_failover_speed(self):
        ...
        mock_openai.classify = AsyncMock(return_value=(DetectionCategory.BENIGN, 0.1, "Result"))
        mock_openai.health_check = AsyncMock(return_value=True)

        with (
            patch(
                "prompt_sentinel.detection.llm_classifier.AnthropicProvider",
                return_value=mock_anthropic,
            ),
            patch(
                "prompt_sentinel.detection.llm_classifier.OpenAIProvider", return_value=mock_openai
            ),
    ...
    async def test_cache_performance(self):
        ...
        with patch(
            "prompt_sentinel.detection.llm_classifier.AnthropicProvider", return_value=mock_provider
        ):
            manager = LLMClassifierManager(config)
            ...
            # Cached call should be faster or at least similar
            # Note: In tests without real LLM calls, the difference may be minimal
            # We're mainly checking that caching doesn't significantly slow things down
            assert cached_call <= first_call * 1.5  # Allow up to 50% slower for test variability
...
class TestThroughput:
    ...
    async def test_max_throughput(self):
        ...
        messages = [Message(role=Role.USER, content=f"Message {i}") for i in range(1000)]
