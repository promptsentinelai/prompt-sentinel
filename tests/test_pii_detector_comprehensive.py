...
class TestPIIDetector:
    ...
    def test_detect_credit_card_valid(self, detector):
        ...
        for match in credit_card_matches:
            assert match.masked_value.endswith("-0366") or \
                   match.masked_value.endswith("-9903") or \
                   match.masked_value.endswith("-0126") or \
                   match.masked_value.endswith("-0009")
    ...
    def test_detect_ssn_various_formats(self, detector):
        ...
        standard_match = next((m for m in ssn_matches if "-" in text[m.start_pos:m.end_pos]), None)
    ...
    def test_detect_aws_keys(self, detector):
        ...
        access_key_match = next((m for m in aws_matches if "AKIA" in text[m.start_pos:m.end_pos]), None)
    ...
    def test_deduplicate_matches_same_position(self, detector):
        ...
        matches = [
            ...
            PIIMatch(PIIType.API_KEY, 10, 20, "***", 0.9, "context2"),  # Same position, higher confidence
        ]
