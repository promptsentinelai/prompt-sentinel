...
class TestWorkflowEngine:
        ...
        workflow_def = {
            ...
            "steps": [
                ...
                },
                {
                    ...
                    "config": {
                        "data_sources": ["threat_intel", "user_context"]
                    }
                },
                {
                ...
                },
                {
                    ...
                    "config": {
                        "actions": ["block_user", "alert_security_team"]
                    }
                }
            ]
        }
    ...
    async def test_workflow_execution(self, workflow_engine):
        ...
        execution = await workflow_engine.execute_workflow(
            ...
            input_data={"detection_id": "test_123"}
        )
    ...
    async def test_error_handling(self, workflow_engine):
        ...
        with patch('prompt_sentinel.automation.workflow.external_api_call', 
            ...
            failing_step = next(
                step for step in status["step_details"] 
    ...
    async def test_workflow_versioning(self, workflow_engine):
        ...
        execution = await workflow_engine.execute_workflow(
            workflow_id=v1_result["workflow_id"],
            version="1.0"
        )
...
class TestWorkflowSteps:
    ...
    async def test_enrichment_step(self, step_executor):
        ...
        input_data = {
            "user_id": "user_123",
            "indicator_value": "malicious_pattern"
        }
        
        # Mock external data sources
        with patch('prompt_sentinel.automation.steps.api_call') as mock_api, \
    ...
    async def test_notification_step(self, step_executor):
        ...
        with patch('prompt_sentinel.automation.steps.send_slack_message') as mock_slack, \
    ...
    async def test_action_step(self, step_executor):
        ...
        action_config = {
            "actions": [
                {
                    "type": "block_user",
                    "config": {"user_id": "{{user_id}}", "duration": "24h"}
                },
                {
...
class TestWorkflowOrchestration:
    ...
    async def test_workflow_coordination(self, orchestrator):
        ...
        execution = await orchestrator.execute_coordinated_workflows(
            ...
            input_data={"content": "suspicious text content"}
        )
...
class TestEventDrivenWorkflows:
        ...
        cep_patterns = [
            {
                ...
                """,
                "action": {
                    "workflow_id": "escalation_response",
                    "priority": "critical"
                }
            }
        ]
    ...
    async def test_event_sourcing(self, event_engine):
        ...
        replay_result = await event_engine.replay_workflow_from_events(
            execution_id=execution["execution_id"],
            target_event_id=event_history[-1]["event_id"]
        )
